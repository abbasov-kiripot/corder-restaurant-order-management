...
export default class TablesOperations {
  ...
  async displayUpdate() {
    ...
    selectedTableNumberOfTicketsElement.innerHTML = `${this.selectedTableNumberOfTickets ?? 0} Xidmət`;
  ...
  clickEventAdder() {
    ...
    this.closeTableButton.addEventListener("click", async () => {
      if (
        confirm(
          `${this.selectedTableArea} - ${this.selectedTableNumber} Masanı bağlamaq istədiyinizə əminsiniz?`
        )
      ) {
        if (confirm("Bu əməliyyat geri qaytarıla bilməz!\nDavam etmək üçün təsdiqləyin.")) {
  ...
  async printTable() {
    if (this.selectedTableNumberOfTickets == undefined) {
      alert("Masa üçün açıq bilet yoxdur.");
      return;
    }

    if (!confirm("Davam etsəniz, masada açılan bütün biletlər çap olunacaq!")) {
      return;
    }
  ...
  async getTablesAreaDisplay() {
    ...
    let display = `<option selected value="string">Bölgə</option>`;
  ...
  async getTableNumbersDisplay() {
    ...
    tableTypesHeaderElement.addEventListener("change", async () => {
      ...
      if (tableData.status_code == "no_session" || tableData == "") {
        tableNumbers.innerHTML = `<option selected value="string">Masa Nömrəsi</option>`;
      }

      let display = `<option selected value="string">Masa Nömrəsi</option>`;
  ...
  movingTableRequest() {
    ...
    movingRegisterButton.addEventListener("click", async () => {
      if (!confirm("Masayı köçürmək istədiyinizə əminsiniz?")) {
        return;
      }

      if (!confirm("Bu hərəkət geri dönməzdir, lütfən, son təsdiqi verin.")) {
        return;
      }
      ...
      if (response.status_code == "success") {
        alert("Masa Daşıma başarılı");

        window.location.href = baseUrl + "tables";
      } else if (response.status_code == "table_is_full") {
        alert("Hədəf masa dolu!");
      } else {
        alert("Masa Daşıma başarısız");
      }
    });
  }
}
